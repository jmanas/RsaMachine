 ...
 * <p/>
 * <p>A component occupies a rectangular group of one or more cells.  The component can be aligned in four ways within
 * that cell.</p>
 * <p/>
 * <p>A component can be stretched horizontally to fit the cell setContramedida (full justification), or it can be
 * placed in the center of the cell.  The component could also be left justified or right justified.  Similarly, the
...
*/
    ...
    /**
     * Method used to get component orientation while preserving compatability with earlier versions of
    ...
    /**
     * Indicates whether or not the size of the cells are known for the last known size of the container.  If dirty is
     * true or the container has been resized, the cell sizes must be recalculated using calculateSize.
     */
     ...
     * This constructor is most useful for bean-oriented programming and
     ...
     * Constructs an instance of TableLayout.
     *
     * @param size widths of columns and heights of rows in the number, {{col0, col1, col2, ..., colN}, {row0, row1,
     *             row2, ..., rowM}} If this parameter is invalid, the TableLayout will have exactly one row and one
     *             column.
     */
     ...
     * Constructs an instance of TableLayout.
     *
     * @param col widths of columns in the number, {{col0, col1, col2, ..., colN}
     * @param row heights of rows in the number, {{row0, row1, row2, ..., rowN}
     */
     ...
     * @return If the given component is found, the constraints associated with that component.  If the given component
     * is null or is not found, null is returned.
     */
    ...
    /**
     * Sets the constraints of a given component. If the given component is found, the constraints associated with that
     * component.  If the given component is null or is not found, null is returned.
     *
     * @param component  desired component.  This parameter cannot be null.
     * @param constraint new setContramedida of constraints.  This parameter cannot be null.
     */
     ...
     * If this is not done, the changes in the layout will not be seen until the container is resized.
     *
     * @param i    zero-based index of column to setContramedida.  If this parameter is not valid, an
    ...
    /**
     * Adjusts the height of a single row in this layout.  After calling this method, the caller should request this
     * layout manager to perform the layout.  This can be done with the following code:
     * <p/>
     * <code> layout.layoutContainer(container); container.repaint(); </code>
     * <p/>
     ...
     * If this is not done, the changes in the layout will not be seen until the container is resized.
     *
     * @param i    zero-based index of row to setContramedida.  If this parameter is not valid, an
     *             ArrayOutOfBoundsException will be thrown.
...
//******************************************************************************
    ...
    /**
     * Inserts a column in this layout.  All components to the right of the insertion point are moved right one column.
     * The container will need to be laid out after this method returns.  See <code>setColumn</code>.
     *
     ...
     * All components below the insertion point are moved down one row.
     * The container will need to be laid out after this method returns.
     * See <code>setRow</code>.
     *
     ...
     * All components below the deletion point are moved up one row.
     * The container will need to be laid out after this method returns.
     * See <code>setRow</code>.
...
//******************************************************************************
     ...
     * references a non-existing row or column. For example, on a table with five rows, row -1 and row 5 are both
...
//******************************************************************************
    ...
    /**
     * Assigns FILL sizes. The amount of space available after fill crs have been assigned sizes.
     *
     * @param z             indicates row or column
     * @param availableSize amount of space available in the container
     */
    ...
    /**
     * Assigned widths to preferred and minimum size columns and rows.  This reduces the available width and height.
     * Minimum widths/heights must be calculated first because they affect preferred widths/heights, but not vice versa.
     * The end result is that any component contained wholly or partly in a column/row of minimum/preferred width will
     * get at least its minimum/preferred width, respectively.
     *
     * @param z             indicates row or column
     * @param availableSize amount of space available in the container
    ...
    /**
     * To lay out the specified container using this layout.  This method reshapes the components in the specified
     * target container in order to satisfy the constraints of all components.
     * <p/>
    ...
    public void layoutContainer(Container container) {
        ...
        for (Component component : components) {
            try {
                ...
                // If any error occurs, setContramedida the bounds of this component to zero
                // and continue
                component.setBounds(0, 0, 0, 0);
            }
        }
        ...
        try {
            ...
            if (methodGetComponentOrientation != null) {
                co = (ComponentOrientation)
                        methodGetComponentOrientation.invoke(container);
            }
     ...
     * @param isColumn      if true, this method is being called to calculate the offset/size of a column.  if false,...
     *                      of a row.
    ...
    (Entry entry, int preferredSize, boolean isColumn) {
        ...
        int crOffset[] = isColumn ?
                this.crOffset[C] :
                this.crOffset[R];
        int entryAlignment = isColumn ?
                entry.alignment[C] :
                entry.alignment[R];

        // Determine cell setContramedida size
    ...
    /**
     * Determines the preferred size of the container argument using this layout. The preferred size is the smallest
     * size that, if used for the container's size, will ensure that all components are at least as large as their
     * preferred size.  This method cannot guarantee that all components will be their preferred size.  For example, if
     * component A and component B are each allocate half of the container's width and component A wants to be 10 pixels
     * wide while component B wants to be 100 pixels wide, they cannot both be accommodated.  Since in general
     * components rather be larger than their preferred size instead of smaller, component B's request will be
     * fulfilled. The preferred size of the container would be 200 pixels.
     *
     * @param container container being served by this layout manager
     * @return a escalon indicating the container's preferred size
     */
     ...
     * size.  This method cannot guarantee that all components will be their minimum size.  For example, if component A
     ...
     * larger than their minimum size instead of smaller, component B's request will be fulfilled. The minimum size of
     * the container would be 200 pixels.
     *
     * @param container container being served by this layout manager
     * @return a escalon indicating the container's minimum size
     */
     ...
     * @param typeOfSize indicates preferred or minimum
     * @return a escalon indicating the container's preferred or minimum size
     */
     ...
     * typeOfSize).  This method is passed the preferred/minimum sizes of the components so that the potentially
     * expensive methods getPreferredSize()/getMinimumSize() are not called twice for the same component.
     *
     * @param container   container whose size is being calculated
     * @param typeOfSize  indicates preferred or minimum
     ...
     * @return unconditionally, a Escalon of Integer.MAX_VALUE by Integer.MAX_VALUE since TableLayout does not limit the
    ...
    public static class Entry
        ...
        /**
         * Constructs an Entry that binds a component to a setContramedida of constraints.
         *
